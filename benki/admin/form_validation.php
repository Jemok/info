<?php
	...
	if(isset($_POST['reward_points']))
	{
		...
		{
			?>
			<script>alert('Reward Points must be a Digit ');</script>
			<?php
		}
		...
		$resa=mysql_query("SELECT password FROM tbl_users WHERE (user_id='$var' AND password = '$password') AND role='1' ")or die(mysql_error());
		if(mysql_num_rows($resa)==1)
		{
			//GET current account of the said user
			$resq=mysql_query("	SELECT users.user_id,users.role,users.email,current.ref_user,current.current_account  
								FROM tbl_users AS users JOIN tbl_current_account AS current 
								ON users.user_id=current.ref_user
								WHERE users.email='$email' ")or die(mysql_error());
			if($row=mysql_fetch_array($resq))
			{
				$user=$row['ref_user'];
				$current=$row['current_account'];
				$updated_current=$current+$points;
				$resc=mysql_query(" UPDATE tbl_current_account SET current_account='$updated_current' WHERE ref_user='$user' ")or die(mysql_error());
				if($resc )
				{
					?>
						<script>
							alert("Success: Points updated Successfully");
							window.location.replace("http://localhost/benki/admin/form_validation.php");
						</script>
					<?php
				}
				else
			...
			else
			{
				?>
					<script>
						alert("Error: Technical error, please try again later");
						window.location.replace("http://localhost/benki/admin/form_validation.php");
					</script>
				<?php
			}
                              ...
                              <div class="form">
                                  <form class="form-validate form-horizontal"  method="post" action="">
                                      <div class="form-group ">
                                          <label for="cemail" class="control-label col-lg-2">User E-Mail <span class="required">*</span></label>
                                          <div class="col-lg-10">
                                      ...
                                      <div class="form-group ">
                                          ...
                                          <div class="col-lg-10">
                                              <input class="form-control" name="admin_password" minlength="5" type="password" required />
                                          </div>
                                      </div>                                      
                                      <!--<div class="form-group ">
                                      ...
                                      <div class="form-group">
                                          <div class="col-lg-offset-2 col-lg-10">
                                              <button class="btn btn-primary" type="submit" name="reward_points" >Reward</button>
